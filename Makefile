...
build-arm64-linux:
	@test -d "$(RELEASE_BUILD_DIR)" || CC="$(CC)" $(MESON) setup "$(RELEASE_BUILD_DIR)" --buildtype=release
	$(NINJA) -C "$(RELEASE_BUILD_DIR)"

.PHONY: build-arm64-linux-debug
build-arm64-linux-debug:
	@test -d "$(DEBUG_BUILD_DIR)" || CC="$(CC)" $(MESON) setup "$(DEBUG_BUILD_DIR)" --buildtype=debug
