...
4. high-value syscall gap and socket ABI coverage (`signalfd4`, SysV semaphores, POSIX mqueues, `memfd_create`, `openat2`, `faccessat2`, `preadv2`, `pwritev2`, `process_vm_*`, UDP `sendto`/`recvfrom`, TCP `listen`/`accept`, socketpair `sendmsg`/`recvmsg`, `getsockname`, `setsockopt`, `getsockopt`);
...
- Latest staged run: **49 / 49 passing** (`/workspace/tmp/ish-arm64-runtime-coverage-20260513-212130.md`, `TIMEOUT_S=180`, `INSTALL_TIMEOUT_S=300`).
