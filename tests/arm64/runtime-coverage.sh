...
int main(){
 ...
 int sp[2]; if(socketpair(AF_UNIX,SOCK_STREAM|SOCK_CLOEXEC,0,sp)<0) die("socketpair"); struct iovec siov[2]={{(char*)"ab",2},{(char*)"cd",2}}; struct msghdr sm={.msg_iov=siov,.msg_iovlen=2}; if(sendmsg(sp[0],&sm,0)!=4) die("sendmsg"); char r1[3]={0},r2[3]={0}; struct iovec riov2[2]={{r1,2},{r2,2}}; struct msghdr rm={.msg_iov=riov2,.msg_iovlen=2}; if(recvmsg(sp[1],&rm,0)!=4||memcmp(r1,"ab",2)||memcmp(r2,"cd",2)) die("recvmsg"); close(sp[0]); close(sp[1]);
