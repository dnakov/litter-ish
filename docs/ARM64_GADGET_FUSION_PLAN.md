...
- First-call-site fixture harness: `make test-arm64-internal-continue-fixtures` now builds a dedicated guest C/assembly fixture and generated `/workspace/tmp/ish-arm64-internal-continue-fixtures-20260516-110252.md` (**6 / 6 passing**). It verifies default/no-stats silence, stats-only default-off `internal_continue=0`, opt-in `B.cond` taken-external plus fallthrough-internal execution (`internal_continue=263`), a call-adjacent/RET-cache smoke (`internal_continue=264`), and precise fault-PC reporting from a load fault inside the internal segment (`internal_continue=269`).
